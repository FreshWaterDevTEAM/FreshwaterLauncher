...
- `inject-version-helper.py` — add `writeVersionFile()`/`hashFileWithDigest()` to staged `arc_dns_injector` / `forge_installer` / `methods_injector_agent` (normally inherited from Amethyst root build.gradle).
